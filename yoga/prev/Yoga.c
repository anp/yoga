...
static inline const YGValue *
YGComputedEdgeValue(const YGValue edges[YGEdgeCount], const YGEdge edge,
...
void YGNodeSetMeasureFunc(const YGNodeRef node, YGMeasureFunc measureFunc) {
  if (measureFunc == NULL) {
    ...
    YGAssertWithNode(node, YGNodeGetChildCount(node) == 0,
                     "Cannot set measure function: Nodes with measure "
...
static inline bool
YGFlexDirectionIsColumn(const YGFlexDirection flexDirection) {
...
static inline YGDirection
YGNodeResolveDirection(const YGNodeRef node,
                       const YGDirection parentDirection) {
...
static void YGNodelayoutImpl(const YGNodeRef node, const float availableWidth,
      ...
      if (child->style.positionType == YGPositionTypeAbsolute &&
        ...
        if (child->style.positionType == YGPositionTypeRelative) {
          ...
          if (canSkipFlex) {
            ...
            crossDim =
                fmaxf(crossDim, YGNodeDimWithMargin(child, crossAxis,
                                                    availableInnerWidth));
          }
      ...
      for (ii = startIndex; ii < childCount; ii++) {
        ...
        if (child->style.positionType == YGPositionTypeRelative) {
          ...
          if (YGNodeAlignItem(node, child) == YGAlignBaseline) {
            ...
            lineHeight = fmaxf(lineHeight, maxAscentForCurrentLine +
  ...
  if (performLayout) {
    ...
    for (currentAbsoluteChild = firstAbsoluteChild;
      ...
      YGNodeAbsoluteLayoutChild(node, currentAbsoluteChild, availableInnerWidth,
                                isMainAxisRow ? measureModeMainDim
                                              : measureModeCrossDim,
...
bool YGLayoutNodeInternal(const YGNodeRef node, const float availableWidth,
  ...
  if (!needToVisitNode && cachedResults != NULL) {
    ...
    if (gPrintChanges && gPrintSkips) {
      YGLog(node, YGLogLevelVerbose, "%s%d.{[skipped] ", YGSpacer(gDepth),
            gDepth);
      if (node->print) {
        node->print(node);
      }
      YGLog(node, YGLogLevelVerbose,
            ...
            YGMeasureModeName(heightMeasureMode, performLayout), availableWidth,
            availableHeight, cachedResults->computedWidth,
            cachedResults->computedHeight, reason);
    }
  } else {
    if (gPrintChanges) {
      YGLog(node, YGLogLevelVerbose, "%s%d.{%s", YGSpacer(gDepth), gDepth,
            needToVisitNode ? "*" : "");
      if (node->print) {
        node->print(node);
      }
...
inline bool
YGConfigIsExperimentalFeatureEnabled(const YGConfigRef config,
                                     const YGExperimentalFeature feature) {
