...
lazy_static! {
    pub static ref ZERO: F32 = unsafe { F32::unchecked_new(0.0) };
}
...
pub(crate) type F32 = ordered_float::NotNaN<f32>;
    ...
    Self: 'static + std::fmt::Debug + Sized,
{
    ...
    fn resolve_dimensions(&mut self) {
        for &dim in [Dimension::Width, Dimension::Height].into_iter() {
            ...
            // if style.maxDimensions[dim] != style.minDimensions[dim] {
            //     *self.resolved() = style.maxDimensions[dim];
            // } else {
    ...
    fn trailing_position(&self, axis: FlexDirection, axis_size: F32) -> F32 {
    ...
    fn margin_for_axis(&mut self, axis: FlexDirection, width_size: F32) -> F32 {
        F32::new(*self.leading_margin(axis, width_size) + self.trailing_margin(axis, width_size))
            .unwrap()
    }

    fn leading_margin(&self, axis: FlexDirection, width_size: F32) -> F32 {
    ...
    fn calculate_layout(
        ...
        let (width, width_measure_mode) =
            if self.is_style_dim_defined(FlexDirection::Row, parent_width) {
                ...
                if *self.style().maxDimensions.width.resolve(parent_width) >= 0.0f32 {
                    (
                        self.style().maxDimensions.width.resolve(parent_width),
            ...
            if self.is_style_dim_defined(FlexDirection::Column, parent_height) {
                (
                    *self.resolved()[FlexDirection::Column.dimension()].resolve(parent_height)
                        + *self.margin_for_axis(FlexDirection::Column, parent_width)
                            .unwrap_or(0.0),
                    Some(MeasureMode::Exactly),
                )
            } else {
                if self.style().maxDimensions.height.resolve(parent_height) >= 0.0f32 {
                    (
                        self.style().maxDimensions.height.resolve(parent_height),
                        Some(MeasureMode::AtMost),
                    )
                } else {
                    (parent_height, None)
    ...
    fn apply_styles<'a, I>(&mut self, styles: I)
    where
    ...
    fn apply_style(&mut self, new_style: &FlexStyle) {
        let mut style = &mut unsafe { (*self.inner_node).style };

        let updated = style.apply_style(new_style);

        if updated::Updated::Dirty == updated {
            self.mark_dirty();
        }
    }

    fn relative_position(&self, axis: FlexDirection, axis_size: F32) -> F32 {
    ...
    fn set_position(
        ...
        let main_axis: FlexDirection = self.style()
            .flexDirection
...
pub unsafe fn YGNodeIsLeadingPosDefined(node: Node, axis: FlexDirection) -> bool {
...
pub unsafe fn YGNodeTrailingMargin(node: Node, axis: FlexDirection, widthSize: c_float) -> c_float {
...
pub unsafe fn YGNodeResolveDirection(node: Node, parentDirection: Direction) -> Direction {
...
pub unsafe fn YGNodeSetChildTrailingPosition(node: Node, child: Node, axis: FlexDirection) -> () {
...
pub unsafe fn YGNodeTrailingBorder(node: Node, axis: FlexDirection) -> c_float {
...
pub unsafe fn YGNodeLeadingBorder(node: Node, axis: FlexDirection) -> c_float {
    if FlexDirectionIsRow(axis)
...
pub unsafe fn YGNodeLeadingPadding(node: Node, axis: FlexDirection, widthSize: c_float) -> c_float {
...
pub unsafe fn YGNodeAlignItem(node: Node, child: Node) -> Align {
...
pub unsafe fn YGNodeIsTrailingPosDefined(node: Node, axis: FlexDirection) -> bool {
...
pub unsafe fn YGNodeBoundAxisWithinMinAndMax(
    ...
    if FlexDirectionIsColumn(axis) {
        min = YGResolveValue(
            &mut (*node).style.minDimensions[Dimension::Height] as *mut Value,
            axisSize,
        );
        max = YGResolveValue(
            &mut (*node).style.maxDimensions[Dimension::Height] as *mut Value,
            axisSize,
        );
    } else {
        if FlexDirectionIsRow(axis) {
            min = YGResolveValue(
                &mut (*node).style.minDimensions[Dimension::Width] as *mut Value,
                axisSize,
            );
            max = YGResolveValue(
                &mut (*node).style.maxDimensions[Dimension::Width] as *mut Value,
                axisSize,
            );
        };
    };
...
pub unsafe fn YGBaseline(node: Node) -> c_float {
...
pub unsafe fn YGNodeIsLayoutDimDefined(node: Node, axis: FlexDirection) -> bool {
    let value: c_float = (*node).layout.measuredDimensions[DIM[axis as usize]];
    return !value.is_nan() && value >= 0.0f32;
}
pub unsafe fn YGIsBaselineLayout(node: Node) -> bool {
...
pub unsafe fn YGNodeDimWithMargin(node: Node, axis: FlexDirection, widthSize: c_float) -> c_float {
...
pub unsafe fn YGMarginLeadingValue(node: Node, axis: FlexDirection) -> *mut Value {
...
pub unsafe fn YGMarginTrailingValue(node: Node, axis: FlexDirection) -> *mut Value {
...
pub unsafe fn YGResolveFlexGrow(node: Node) -> c_float {
...
pub const kDefaultFlexGrow: c_float = 0.0f32;

pub unsafe fn YGNodeResolveFlexShrink(node: Node) -> c_float {
    if (*node).parent.is_null() {
        return 0.0;
    };
...
pub const kDefaultFlexShrink: c_float = 0.0f32;
pub const kWebDefaultFlexShrink: c_float = 1.0f32;
pub unsafe fn YGNodeIsFlex(node: Node) -> bool {
...
pub unsafe fn YGNodeResolveFlexBasisPtr(node: Node) -> *const Value {
    if (*node).style.flexBasis.unit != UnitType::Auto
        && (*node).style.flexBasis.unit != UnitType::Undefined
    {
        return &mut (*node).style.flexBasis as *mut Value;
    };
...
pub unsafe fn YGZeroOutLayoutRecursivly(node: Node) -> () {
    memset(
...
pub unsafe fn YGNodeMarkDirty(node: Node) -> () {
    YGAssertWithNode(
        node,
        (*node).measure.is_some(),
...
pub unsafe fn YGNodeIsDirty(node: Node) -> bool {
    return (*node).isDirty;
}

pub unsafe fn YGNodeCopyStyle(dstNode: Node, srcNode: Node) -> () {
    if memcmp(
        &mut (*dstNode).style as *mut Style_0 as *const c_void,
        &mut (*srcNode).style as *mut Style_0 as *const c_void,
        size_of::<Style_0>(),
    ) != 0i32
    {
        memcpy(
            &mut (*dstNode).style as *mut Style_0 as *mut c_void,
            &mut (*srcNode).style as *mut Style_0 as *const c_void,
            size_of::<Style_0>(),
        );
...
pub unsafe fn YGNodeSetContext(node: Node, mut context: *mut c_void) -> () {
    (*node).context = context;
}

pub unsafe fn YGNodeGetContext(node: Node) -> *mut c_void {
    return (*node).context;
}

pub unsafe fn YGNodeSetMeasureFunc(node: Node, mut measureFunc: YGMeasureFunc) -> () {
...
pub unsafe fn YGNodeGetMeasureFunc(node: Node) -> YGMeasureFunc {
    return (*node).measure;
}

pub unsafe fn YGNodeSetBaselineFunc(node: Node, mut baselineFunc: YGBaselineFunc) -> () {
    (*node).baseline = baselineFunc;
}

pub unsafe fn YGNodeGetBaselineFunc(node: Node) -> YGBaselineFunc {
    return (*node).baseline;
}

pub unsafe fn YGNodeSetHasNewLayout(node: Node, mut hasNewLayout: bool) -> () {
    (*node).hasNewLayout = hasNewLayout;
}

pub unsafe fn YGNodeGetHasNewLayout(node: Node) -> bool {
    return (*node).hasNewLayout;
}

pub unsafe fn YGNodeSetNodeType(node: Node, mut nodeType: NodeType) -> () {
    (*node).nodeType = nodeType;
}

pub unsafe fn YGNodeGetNodeType(node: Node) -> NodeType {
    ...
    for i in 0..childCount {
        ...
        while endOfLineIndex < childCount {
            ...
            if measureModeMainDim == MeasureMode::AtMost && remainingFreeSpace > 0.0 {
                if (*node).style.minDimensions[DIM[mainAxis as usize]] != None
                    && YGResolveValue(
