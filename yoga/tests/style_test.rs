...
use yoga::{Direction, FlexDirection, Node, Undefined, Value};
...
fn test_copy_style_modified() {
    ...
    assert_eq!(node0.get_style_max_height(), Value::UndefinedValue);

    let mut node1 = Node::new();
    node1.set_flex_direction(FlexDirection::Row);
    node1.set_max_height(Value::Point(10.0.into()));

    node0.copy_style(&node1);
    assert!(node0.is_dirty());
    assert_eq!(FlexDirection::Row, node0.get_flex_direction());
    assert_eq!(node0.get_style_max_height(), Value::Point(10.0.into()));
}
...
fn test_copy_style_modified_same() {
    ...
    node0.set_max_height(Value::Point(10.0.into()));
